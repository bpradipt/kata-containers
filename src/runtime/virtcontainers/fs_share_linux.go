...
func NewFilesystemShare(s *Sandbox) (FilesystemSharer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to add watcher")
	}
...
func (f *FilesystemShare) ShareFile(ctx context.Context, c *Container, m *Mount) (*SharedFile, error) {
	...
	if !caps.IsFsSharingSupported() {
		...
		walk := func(srcPath string, d fs.DirEntry, err error) error {
			...
			f.Logger().Infof("ShareFile: Copying file from src (%s) and dest (%s)", srcPath, dstPath)
...
func (f *FilesystemShare) watchDir(source string) error {
	...
	if len(watchList) == 0 {
		f.Logger().Infof("watchDir: Adding fsnotify watcher for dir (%s)", source)
		err := f.watcher.Add(source)
		if err != nil {
			f.Logger().Info("watchDir: Failed to add watcher to list")
			return err
		}
	}
